...
(function () {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === 'GET_SELECTED_TEXT') {
      ...
      // Convert selected HTML to markdown if we have Turndown loaded
      let selectedMarkdown = selectedText;
      if (selectedHtml && typeof TurndownService !== 'undefined') {
        try {
  ...
  function createTurndown() {
    ...
    td.remove(['script', 'style', 'noscript', 'iframe', 'svg', 'canvas', 'nav', 'footer']);
  ...
  async function getPageContent() {
    ...
    clone.querySelectorAll('[hidden], [aria-hidden="true"], [style*="display:none"], [style*="display: none"]').forEach((el) => el.remove());
